...
assert(weekIdFromStart(start) === '2026-08-27', `weekId=${weekIdFromStart(start)}`);
...
assert(weekIdFromStart(startFri) === '2026-08-27', `fri week=${weekIdFromStart(startFri)}`);
assert(isAccepting(friday, startFri), 'Friday should be accepting');

// 木曜 09:59 JST は前週
const beforeThu = Date.parse('2026-08-27T00:59:00.000Z');
const startPrev = weekStartUtc(beforeThu);
assert(weekIdFromStart(startPrev) === '2026-08-20', `prev=${weekIdFromStart(startPrev)}`);
